...
export async function searchDocumentsOptimized(params: SearchParams) {
  ...
  try {
    ...
    console.log(`[SEARCH] Query: "${q}", Cities: ${cities?.length || 0}, Params: ${params_array.length}`);

    const rows = await prisma.$queryRawUnsafe(sql, ...params_array) as any[];
    ...
    const fallbackRows = await prisma.document.findMany({
      where: {
        OR: [
...
export async function warmUpCache() {
  ...
  for (const query of popularQueries) {
    try {
      await searchDocumentsOptimized({ q: query });
